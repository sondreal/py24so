...
from py24so.models.customer import Address, Contact, CustomerCreate
from py24so.models.invoice import InvoiceCreate, InvoiceLineItem, InvoiceStatus, InvoiceUpdate
from py24so.models.product import PriceInfo, ProductCreate
...
def test_product(client):
    ...
    product_data = ProductCreate(
        ...
        price_info=PriceInfo(price=299.99, currency="NOK", vat_rate=25.0, unit="pcs"),
        is_service=False,
        is_active=True,
    )
...
def test_invoice_crud(client, test_invoice_data):
    ...
    # Update the invoice
    update_data = InvoiceUpdate(notes="Updated integration test invoice", reference="TEST-REF-123")
    ...
    filtered_invoices = client.invoices.list(
        status=InvoiceStatus.DRAFT, customer_id=test_invoice_data.customer_id
    )
    ...
    # Mark as paid
    paid_invoice = client.invoices.mark_as_paid(invoice_id, payment_date=date.today().isoformat())
